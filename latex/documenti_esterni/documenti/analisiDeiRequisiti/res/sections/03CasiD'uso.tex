...
\subsubsection{UC5 - Registrazione all'Issuer}
\begin{itemize}
\item \textbf{Attore principale:} Holder.
...
\item \textbf{Scenario principale:} L'utente (\textit{holder}) ha eseguito con successo la registrazione alla piattaform Issuer.
\item \textbf{Scenario alternativo:} L'utente (\textit{holder}) non è riuscito ad eseguire la registrazione.
\item \textbf{estensione:} UC7-Visualizzazione Errore.
\end{itemize}

\subsubsection{UC6 - Login all'Issuer}
\begin{itemize}
\item \textbf{Attore principale:} Holder.
\item \textbf{Attore secondario}: Issuer (sistema). 
\item \textbf{Precondizioni:} L'utente (\textit{holder}) inserisce le credenziali di accesso ovvero email e password.
\item \textbf{Postcondizioni:} L'utente (\textit{holder}) una volta inserite le credenziali è riuscito a fare l'accesso al Issuer ed è entrato nel sistema.
...
\subsubsection{UC7 - Logout dal Wallet}
\begin{itemize}
\item \textbf{Attore principale:} Holder.
\item \textbf{Attore secondario:} Issuer (sistema).
\item \textbf{Precondizioni:} L'utente (\textit{holder}) vuole effettuare il logout dalla piattaforma Issuer.
...
\item l'\textit{holder} si registra e/o esegue il login nel portale \textit{Issuer};
...
\item \textbf{Scenario principale:} 
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \item placeOfBirth.
        \end{itemize}
        \item l' \textit{holder} ,una volta effettuato il login nel portale dell'Issuer, attende che la sua richiesta venga esaminata dall'\textit{Issuer (admin)}
    \end{enumerate}
\end{itemize}
